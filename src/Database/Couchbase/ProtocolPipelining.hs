...
--      subsequent requests.
--
module Database.Couchbase.ProtocolPipelining (
  Connection(..)
  , connect
--  , enableTLS
  , beginReceiving
  , disconnect
--  , request, send, recv, flush
  , handle
  , fromCtx
  ) where
...
handle :: Connection -> Raw.Lcb
handle conn = lcb
  where (CC.NormalHandle lcb) = connCtx conn
...
connGetReplies conn@Conn{..} = undefined
{-
